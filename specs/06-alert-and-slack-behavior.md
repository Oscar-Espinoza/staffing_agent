...
optional DATA QUALITY NOTES. Each finding includes its title, deterministic detail, and a compact
`Sources:` line. Full rationale remains in the structured findings. Record ids support a finding;
they never dominate it, and they are plain text because the source API exposes no per-record URL to
link to. Header counts are computed from what is actually rendered, so they can never disagree with
the body.
...
quiet rather than returning nothing. `GET /run?dry=1` runs the same analysis with no side effects,
`GET /run?dry=1&demo=1` adds the linking demo of spec 07, `GET /last` shows the last run in a
browser, and `GET /health` answers liveness without touching any dependency.
