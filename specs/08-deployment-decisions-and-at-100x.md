...
A Deno HTTP server exposes `/`, `/health`, `/run`, and `/last`. `GET /run` is the only way a check
starts — there is no scheduler, so a run happens when a reviewer asks for one. `/last` shows the
most recent result in that isolate and each completed run writes a structured log line. There is no
database, queue, authentication, persistent deduplication, or new/worsening detection.
...
2. Persist findings to detect changes and suppress unchanged alerts across runs.
